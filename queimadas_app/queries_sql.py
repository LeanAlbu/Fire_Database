...
# 1. Interseção
def get_intersecao_municipio_regiao():
...
# 2. Diferença
def get_diferenca_municipio_regiao():
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT nome_municipio FROM queimadas_app_municipio
            EXCEPT
...
# 5. IS NOT NULL
def get_municipios_com_populacao():
    with connection.cursor() as cursor:
        cursor.execute("""
...
# 6. IN
def get_municipios_com_focoqueimada():
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT nome_municipio
            FROM queimadas_app_municipio
...
# 7. ANY
def get_municipios_populacao_maior_que_qualquer_area_500():
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT nome_municipio
            FROM queimadas_app_municipio
...
# 8. ALL
def get_municipios_populacao_maior_que_todos_area_200():
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT nome_municipio
            FROM queimadas_app_municipio
...
# 9. EXISTS
def get_municipios_existe_focoqueimada():
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT nome_municipio
...
# 11. AGREGACAO (AVG, MIN, MAX, SUM, COUNT)
def get_agregados_municipio():
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT
